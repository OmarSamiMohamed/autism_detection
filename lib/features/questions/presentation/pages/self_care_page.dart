...
class _SelfCarePageState extends State<SelfCarePage> {
  ...
  final List<String> questions = [
    ...
    "هل الطفل غير قادر على التنقل بمفرده؟",
    "هل تلاحظ أن الطفل معتمد على الآخرين بشكل كلي؟",
    "هل الطفل قادر على إدراك الوقت أو المال؟",
    "هل الطفل يقوم بغسل يديه (مثلاً بعد تناول الطعام)؟",
    "هل تلاحظ أن الطفل لا يستطيع الاستقلالية في أغلب الأمور؟",
  ];
  ...
  Future<void> submitAnswers() async {
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.pushReplacementNamed(context, AppRouter.attention);
    } catch (e) {
      Fluttertoast.showToast(
